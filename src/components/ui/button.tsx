...
const buttonVariants = cva(
  ...
  transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none shrink-0 [&_svg]:shrink-0
   outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20
    dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive active:translate-y-0.5`,
  {
    variants: {
      variant: {
...
const memoizedButton = React.memo(function Button({
  ...
  return (
    <Comp
      data-slot="button"
      className={cn(noStyles ? className : buttonVariants({ variant, size, className }))}
      {...props}
    />
  );
});
